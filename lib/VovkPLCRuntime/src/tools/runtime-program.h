...
public:
    ...
    RuntimeError push(uint8_t* code, uint16_t code_size) {
        if (program_size + code_size > MAX_PROGRAM_SIZE) return PROGRAM_SIZE_EXCEEDED;
    ...
    RuntimeError push(uint8_t instruction) {
        if (program_size + 1 > MAX_PROGRAM_SIZE) return PROGRAM_SIZE_EXCEEDED;
    ...
    RuntimeError push(uint8_t instruction, uint8_t data_type) {
        if (program_size + 2 > MAX_PROGRAM_SIZE) return PROGRAM_SIZE_EXCEEDED;
        program[program_size] = instruction;
    ...
    RuntimeError pushU16(uint16_t value) {
        if (program_size + 3 > MAX_PROGRAM_SIZE) return PROGRAM_SIZE_EXCEEDED;
    ...
    RuntimeError pushU32(uint32_t value) {
        if (program_size + 5 > MAX_PROGRAM_SIZE) return PROGRAM_SIZE_EXCEEDED;
    ...
    RuntimeError pushU64(uint64_t value) {
        if (program_size + 9 > MAX_PROGRAM_SIZE) return PROGRAM_SIZE_EXCEEDED;

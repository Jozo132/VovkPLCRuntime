...
const main = async () => {
    ...
    console.log(`Compiling ${file_names.length} test source file${file_names.length === 1 ? '' : 's'}...`)
    const t = +new Date()
    ...
    console.log(`Testing WASM file${file_names.length === 1 ? '' : 's'}...`)
    ...
    console.log(` Number column format is "${fg_white + bg_red}+diff${c_reset} (used/allocated)"`)
    console.log()
    ...
    const col_pad_number = x => {
        ...
        if (Array.isArray(x)) {
            ...
            const used = x[1]
            const allocated = x[2]
            const bg = diff > 0 ? bg_red : diff < 0 ? bg_dark_green : ''
            const output = fg_white + bg + `${diff > 0 ? '+' : ''}${diff}${c_reset} (${used}/${allocated})`
            x = output
        }
    ...
    console.log(`${''.padEnd(table_offset, ' ')}   ${['2x info', '2x info', '2x info', `${G}empty${R} + 2x info`, `${C}job${R} + 2x info`, '2x info', `${C}job${R} 2x info`, '2x info'].map(col_pad_number).join('   ')}`)
    ...
    for (let i = 0; i < file_names.length; i++) {
        ...
        let last_usage = get_used_memory()
        let total_usage = 0;
        let last_allocated = get_total_memory()
        ...
        const log_memory = () => {
            ...
            usage_allocation_changes.push([change_usage, total_usage, total_allocated])
        }
        // Begin testing
        log_memory() // Log initial memory usage
        log_memory() // Log memory again to make sure it's not changing
        log_memory() // And log it again to find any runtime function call memory leak
        const runtime_leak = usage_allocation_changes[2][0] > 0 // If the memory usage increased after the 3rd log_memory() call, then there is a runtime memory leak

...
const FSH* OPCODE_NAME(PLCRuntimeInstructionSet opcode) {
    switch (opcode) {
        ...
        case TON_CONST: return F("TON_CONST");
        case TON_MEM: return F("TON_MEM");
        case TOF_CONST: return F("TOF_CONST");
...
u8 OPCODE_SIZE(PLCRuntimeInstructionSet opcode) {
    ...
    switch (opcode) {
        ...
        case COMMS: return 0; // Dynamic size: 2 + comms_subfn_param_size(sub_fn) (handled specially in explain)
